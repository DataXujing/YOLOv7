    ...
    for anna_file in files:
        ...
        for obj in root.findall('object'):
            ...
            if (float(bbox.find('ymin').text) >= float(bbox.find('ymax').text)) or (float(bbox.find('xmin').text) >= float(bbox.find('xmax').text)):
                print(anna_file)
                print((bbox.find('ymin').text,bbox.find('ymax').text,bbox.find('xmin').text,bbox.find('xmax').text))
                i += 1
                try:
                    shutil.move(path+"/"+anna_file,"./error2/"+anna_file)
                    shutil.move("./JPEGImages/"+anna_file.split(".")[0]+".jpg","./error2/"+anna_file.split(".")[0]+".jpg")
                except:
                    pass
