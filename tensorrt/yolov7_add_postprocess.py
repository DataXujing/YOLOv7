...
graph = gs.import_onnx(onnx.load("./tensorrt/last.onnx"))


# 添加计算类别概率的结点

# input
origin_output = [node for node in graph.nodes if node.name == "Concat_318"][0]
...
ends_conf = gs.Constant("ends_conf",values=np.array([1,25200,9],dtype=np.int64))
...
label_conf_0 = gs.Variable(name='label_conf_0',shape=(1,25200,4),dtype=np.float32)
...
label_conf = gs.Variable(name='label_conf',shape=(1,25200,4),dtype=np.float32)
...
#----处理prob
scores = gs.Variable(name="scores",shape=(1,25200,4),dtype=np.float32)
